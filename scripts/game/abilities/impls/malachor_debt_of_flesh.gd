...
func cast(ctx: AbilityContext) -> bool:
	...
	if target_index < 0:
		return false
	if not ctx.schedule_implementation_callback(self, "_snap_chain", CHAIN_DURATION, [], "_cancel_chain"):
		return false
	_active_ctx = ctx
	_chained_team = _enemy_team(ctx.caster_team)
	_chained_index = target_index
	_stored_damage = 0
	_redirect_pct = ctx.scale_power(REDIRECT_PCT)
	if ctx.buff_system != null:
		ctx.apply_cc_tag(_chained_team, target_index, "root", CHAIN_DURATION, {})
		ctx.buff_system.record_debuff(ctx.state, _chained_team, target_index, "malachor_flesh_chain", {"redirect_pct": _redirect_pct}, _redirect_pct, CHAIN_DURATION)
	...
	ctx.emit_redirect_semantic(_chained_team, target_index, "malachor_flesh_chain", CHAIN_DURATION, _redirect_pct, CHAIN_DURATION)
...
func _on_hit_applied(source_team: String, source_index: int, victim_index: int, _rolled_damage: int, dealt_damage: int, _crit: bool, _before_hp: int, _after_hp: int, _player_cd: float, _enemy_cd: float) -> void:
	...
	if caster == null or victim == null or not caster.is_alive():
		return
	var redirected: int = min(int(caster.hp) - 1, int(round(float(dealt_damage) * _redirect_pct)))
	if redirected <= 0:
		return
	_redirecting = true
	_active_ctx.heal_from_dealt(_active_ctx.caster_team, victim_index, float(redirected))
...
func _snap_chain() -> void:
	...
	if caster != null and caster.is_alive() and target != null and target.is_alive() and _stored_damage > 0:
		AbilityEffects.damage_single(_active_ctx.engine, _active_ctx.state, _active_ctx.caster_team, _active_ctx.caster_index, _chained_index, float(_stored_damage), "magic")
		_active_ctx.heal_from_dealt(_active_ctx.caster_team, _active_ctx.caster_index, float(_stored_damage) * SNAP_HEAL_PCT)
	_cancel_chain()

func _cancel_chain() -> void:
	if _active_ctx != null:
		var callback: Callable = Callable(self, "_on_hit_applied")
