...
func cast(ctx: AbilityContext) -> bool:
	...
	if killed and not already_awarded:
		var awarded: int = int(Economy.add_stake_units(1, true, "mara_arcane_ledger"))
		if ctx.buff_system != null:
			ctx.buff_system.add_stack(ctx.state, ctx.caster_team, ctx.caster_index, STAKE_AWARDED_KEY, 1)
		ctx.log("Arcane Ledger: %s (1 Stake), capped this combat" % BloodBuckets.format_delta(awarded))
	return true
