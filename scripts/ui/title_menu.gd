...
func _render_how_to_play() -> void:
	...
	_add_card("3. Spend Blood in the Shop", "After the opener, the shop offers five units. Buy selectively from the house blood reserve, reroll when you need a different lane, lock valuable offers, and buy XP to raise your level, add board slots, and improve shop odds. CAPITAL marks the premium current-grade recruit.", "shop blood reserve offers reroll lock xp level board slots odds buy unit capital current grade")
	...
	_add_card("6. Shop, Then Wager", "After shopping, choose a blood wager with visible win odds and after-win or after-loss reserve. Shop purchases reduce what you can risk next, and the wager locks when combat starts.", "blood wager shop planning win odds after win after loss reserve lock")
