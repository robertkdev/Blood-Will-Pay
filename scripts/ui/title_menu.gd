...
func ensure_settings_surface_visible() -> void:
	...
	if _content_scroll != null:
		...
		_content_scroll.modulate = Color.WHITE
		_content_scroll.self_modulate = Color.WHITE
	var selector: OptionButton = find_child("UIScaleOption", true, false) as OptionButton
	if selector != null:
		var popup: PopupMenu = selector.get_popup()
		if popup != null:
			popup.hide()
		selector.set_meta("pressed_state_shell_preserved", true)
