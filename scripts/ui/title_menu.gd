...
func _render_how_to_play() -> void:
	...
	_add_card("3. Spend Blood Buckets in the Shop", "After the opener, the shop offers five units. Buy selectively from the house blood-bucket reserve, reroll when you need a different lane, lock valuable offers, and buy XP to raise your level, add board slots, and improve shop odds. CAPITAL marks the premium current-grade recruit.", "shop blood bucket reserve offers reroll lock xp level board slots odds buy unit capital current grade")
	...
	_add_card("6. Shop, Then Wager", "After shopping, choose how many blood buckets to wager with visible win odds and after-win or after-loss reserve. Shop purchases reduce what you can risk next, and the bucket wager locks when combat starts.", "blood buckets wager shop planning win odds after win after loss reserve lock")
