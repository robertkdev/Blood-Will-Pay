...
func set_entry_presentation_progress(progress: float) -> void:
	# Combat readouts are part of the shared field presentation. Preparing them
	# before spatial movement prevents a late health/focus layer pop.
	...
	set_meta("one_arena_entry_progress", clampf(progress, 0.0, 1.0))
