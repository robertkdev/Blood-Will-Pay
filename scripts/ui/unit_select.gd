...
func _on_resized() -> void:
	...
	if details_scroll != null:
		details_scroll.custom_minimum_size = Vector2(right_width, 118.0 if compact else 126.0)
	if details_label != null:
...
func _style_start_button() -> void:
	if start_button == null:
		return
	var compact: bool = _is_compact_layout()
	var responsive_width: float = 500.0
	if right_column != null and right_column.custom_minimum_size.x > 1.0:
		responsive_width = right_column.custom_minimum_size.x
	start_button.custom_minimum_size = Vector2(responsive_width, 54.0 if compact else 68.0)
