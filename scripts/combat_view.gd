...
func _sync_compact_resource_strip() -> void:
	...
	if not tight_compact:
		return
	var gold_text: String = gold_label.text.strip_edges().to_upper() if gold_label != null else "BLOOD RESERVE --"
	gold_text = gold_text.replace(":", "")
	var progress_text: String = _compact_progress_text()
	strip.text = "%s  //  %s" % [gold_text, progress_text]
	strip.tooltip_text = "Current spendable blood reserve, command level, and XP progress remain visible at enlarged UI scales."
	strip.set_meta("decision_data_complete", gold_text.contains("BLOOD") and progress_text.contains("LVL") and progress_text.contains("XP"))
