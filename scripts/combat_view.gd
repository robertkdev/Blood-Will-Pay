...
func _apply_responsive_layout() -> void:
	...
	_set_minimum_size("MarginContainer/VBoxContainer/BattleArea/ContentRow/LeftItemArea", Vector2(112.0 if maximum_scale_layout else 136.0 if tight_compact else 180.0 if compact else 286.0, 190.0 if tight_compact else battle_height if compact else 596.0))
	_set_minimum_size("MarginContainer/VBoxContainer/BattleArea/ContentRow/LeftItemArea/ItemStorageHeader", Vector2(112.0 if maximum_scale_layout else 136.0 if tight_compact else 172.0 if compact else 286.0, 18.0 if tight_compact else 22.0 if compact else 24.0))
	_set_minimum_size("MarginContainer/VBoxContainer/BattleArea/ContentRow/LeftItemArea/ItemStorageGrid", Vector2(112.0 if maximum_scale_layout else 136.0 if tight_compact else 172.0 if compact else 286.0, 60.0 if tight_compact else 82.0 if compact else 156.0))
	_set_minimum_size("MarginContainer/VBoxContainer/BattleArea/ContentRow/LeftItemArea/TraitsPanel", Vector2(112.0 if maximum_scale_layout else 136.0 if tight_compact else 172.0 if compact else 286.0, 108.0 if tight_compact else 208.0 if compact else 394.0))
	...
	if planning_actions_row != null:
		...
		planning_actions_row.custom_minimum_size = Vector2(0.0 if tight_compact else 900.0 if compact else 1120.0, 46.0 if tight_compact else 48.0 if compact else 56.0)
	...
	wager_summary.custom_minimum_size = Vector2(0.0, 24.0 if tight_compact else 26.0 if compact else 0.0)
...
func _apply_board_tile_size(compact: bool, tight_compact: bool, large_planning_field: bool) -> void:
	...
	var tile_size: Vector2 = Vector2(148.0, 74.0) if ultrawide_planning_field else Vector2(64.0, 30.0) if wide_tight_field else Vector2(46.0, 28.0) if tight_compact else Vector2(124.0, 62.0) if large_planning_field else Vector2(68.0, 50.0) if compact else Vector2(96.0, 76.0)
...
func _apply_side_panel_layout(compact: bool, tight_compact: bool) -> void:
	...
	if controller != null and controller.traits_presenter != null:
		controller.traits_presenter.set_compact_layout(
			112.0 if maximum_scale_layout else 136.0 if tight_compact else 172.0 if compact else 286.0,
		...
		for child: Node in shop_grid.get_children():
			...
			if control != null:
				if bool(control.get_meta("opening_fight_placeholder", false)):
					...
					if price_label != null:
						price_label.add_theme_font_size_override("font_size", 18 if tight_compact else 20)
...
func _apply_planning_action_hierarchy(compact: bool, tight_compact: bool) -> void:
	...
	if planning_directive != null:
		...
		planning_directive.add_theme_font_size_override("font_size", 19 if not compact else 17)
...
func _apply_compact_commit_rail(compact: bool, tight_compact: bool) -> void:
	...
	if compact:
		...
		wager_summary.add_theme_font_size_override("font_size", 13 if tight_compact else 15)
