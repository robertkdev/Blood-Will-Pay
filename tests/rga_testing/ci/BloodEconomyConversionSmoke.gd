...
const REQUIRED_PLAYER_COPY: Dictionary[String, String] = {
	...
	"res://scripts/ui/title_menu.gd": "Spend Blood in the Shop",
...
func _run() -> void:
	...
	if failures.is_empty():
		print("BloodEconomyConversionSmoke: PASS roster=51 internal_gold_boundary=preserved")
		get_tree().quit(0)
		return
...
func _validate_blood_wager_contract(failures: Array[String]) -> void:
	...
	if get_tree().root.get_node_or_null("/root/Economy") == null:
		return
	Economy.reset_run()
	_expect(int(Economy.gold) == 3, "blood reserve should retain tuned starting value 3", failures)
	...
	_expect(int(Economy.gold) == 5, "winning wager 2 should pay out to reserve 5", failures)
	Economy.reset_run()
