...
func _ready() -> void:
	...
	_expect(is_equal_approx(restored.pit_payout_multiplier, 1.25), "Pit should expose its explicit 25-percent encounter-tier payout modifier")
	...
	_expect(is_equal_approx(pit_restored.pit_payout_multiplier, 1.25), "Pit payout modifier should round-trip through save data")
	...
	economy.set_projected_win_probability(float(odds_before) / 100.0)
	var quote_before: float = economy.gross_payout_multiplier()
	economy.set_projected_win_probability(float(odds_after) / 100.0)
	var quote_after: float = economy.gross_payout_multiplier()
	_expect(is_equal_approx(quote_after, quote_before), "harder projected odds must not reprice the wager")
	economy.set_payout_modifier(restored.pit_payout_multiplier)
	var pit_quote: float = economy.gross_payout_multiplier()
	_expect(is_equal_approx(pit_quote, quote_before * restored.pit_payout_multiplier), "Pit modifier should compose with the documented encounter-tier payout quote")
	economy.set_encounter_quote_kind("BOSS")
	_expect(economy.gross_payout_multiplier() > pit_quote, "a higher encounter tier should raise the deterministic payout quote")
	_finish()
