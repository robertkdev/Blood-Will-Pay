...
func _run() -> void:
	...
	_expect(card.get_theme_stylebox("normal") is StyleBoxTexture, "shop card should retain its material frame style")
	...
	_expect(card.get_global_rect().is_equal_approx(hover_rect), "shop hover motion should not change card geometry")
