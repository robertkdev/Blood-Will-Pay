...
func _run() -> void:
	...
	_expect(onset_painter != null and int(onset_painter.get_meta("player_scale_evidence_count", 0)) >= 6 and float(onset_painter.get_meta("terrain_floor_lift", 0.0)) >= 0.18 and String(onset_painter.get_meta("central_collision_read", "")) == "threatened_intact_ground", "onset must remain intact while visibly anticipating a central collision")
	...
	_expect(onset_painter != null and bool(onset_painter.get_meta("central_collision_visible", false)) and int(onset_painter.get_meta("player_scale_evidence_count", 0)) >= 10 and float(onset_painter.get_meta("terrain_floor_lift", 0.0)) >= 0.26 and String(onset_painter.get_meta("central_collision_read", "")) == "impact_crater_drag_and_residue", "midfight must expose a central crater, opposing drag gouges, and casualty residue")
	...
	_expect(onset_painter != null and bool(onset_painter.get_meta("central_collision_visible", false)) and int(onset_painter.get_meta("player_scale_evidence_count", 0)) >= 14 and float(onset_painter.get_meta("terrain_floor_lift", 0.0)) >= 0.33, "collapse did not become materially denser than onset")
	...
	_expect(pressure_painter != null and bool(pressure_painter.get_meta("central_collision_visible", false)) and int(pressure_painter.get_meta("player_scale_evidence_count", 0)) >= 12 and float(pressure_painter.get_meta("terrain_floor_lift", 0.0)) >= 0.26 and String(pressure_painter.get_meta("physical_lane_encroachment", "")) == "static_collision_aftermath" and String(pressure_painter.get_meta("central_collision_read", "")) == "static_rupture_and_drag", "Reduced Motion must retain a distinct, static central collision composition")
