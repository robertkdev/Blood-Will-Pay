...
func _validate_default_preview() -> void:
	...
	if win_odds_label != null:
		_expect(String(win_odds_label.text).begins_with("WIN ") and String(win_odds_label.text).contains("-"), "procedural default odds label should show an estimated range, got %s" % win_odds_label.text)
